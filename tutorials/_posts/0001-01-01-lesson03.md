...
All widgets are derived from `emp::Widget` and structured such that multiple widget objects can properly refer to and modify the same underlying DOM component.
...
:bangbang:
Be sure to remember to do this `source` chore every time you start a new terminal session and want to use Emscripten.
...
Then, start off your `source/web/evo-algo-web.cpp` with something like this.
```cpp
#include "emp/web/web.hpp"
...
The command `make web` will compile `source/web.cpp` for you.

:bangbang:
Rember to `source` Emscripten before you try to compile with `make web`.

:bug:
Compiling with the `make web-debug` command activates debugging features, like additional safety checks and better (human-readable-ish) backtraces on crash.
