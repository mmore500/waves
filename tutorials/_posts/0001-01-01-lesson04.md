...
In order to compile for web, you’ll need the Emscripten LLVM-to-Web Compiler.
...
in order to load Emscripten’s odds and ends into your `PATH`.
You only need to do the first time you want to use Emscripten in a particular terminal session.
...
Now, let's make some starter code.
...
```
emcc -std=c++17 -I../Empirical/source/ -Os --js-library ../../source/web/library_emp.js -s EXPORTED_FUNCTIONS="['_main', '_empCppCallback']" -s NO_EXIT_RUNTIME=1 main.cc -o main.js
```

We should now have `main.js` and `main.js.mem` ready to go.
...
Python provides a handy, no-hassle too serve a website locally.
...
int main() {
  ...
  doc << emp::web::Image("url.goes.here") << "<br>";
...
Can you figure out how to add an [`emp::web::TextArea`](https://github.com/devosoft/Empirical/blob/master/source/web/TextArea.h) that uses `std::cout` to print whatever the user enters to the console?
...
For this section, you'll need to `#include "web/CanvasShapes.h"`.
...
Can you figure out how to draw an [`emp::web::CanvasRect`](https://github.com/devosoft/Empirical/blob/53fa17fb9dad847b42d9f087d2eef332ccf78cad/source/web/CanvasShape.h#L94)?
...
Under the hood, (AFAIK) only processes events one at a time.
...
class PongAnimator : emp::web::Animate {
  ...
  // where we'll draw
  emp::web::Canvas canvas{1000, 100, "canvas"};

public:

  // shove canvas into the div
  PongAnimator() { doc << canvas; }
...
Can you add a "Step" button that calls `animator`'s `DoFrame`?

## Go, Dog. Go!

![sparkle]({{ site.baseurl }}/assets/gifcities-dog.gif){:width="100%"}
...
Can you also show the live numerical values of best and average fitness?
Can you make it look pretty with a little Bootstrap?
